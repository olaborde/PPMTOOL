...
function onSupplyRandomPhoto(context) {
  ...
  items.forEach(function (item, index) {
    return setImageFor(item, index, dataKey);
  });
}
function onSearchPhoto(context) {
  var dataKey = context.data.key;
  var searchTerm = UI.getStringFromUser('Search Unsplash for…', 'People').replace(' ', '-').toLowerCase();

  if (searchTerm != 'null') {
    var items = util.toArray(context.data.items).map(sketch.fromNative);
    items.forEach(function (item, index) {
      return setImageFor(item, index, dataKey, searchTerm);
    });
  }
}
